...
`options` entries are plain strings or `{ "value", "title" }`. Defaults are validated against the options, sliders against `min` and `max`, and every error names the item.
