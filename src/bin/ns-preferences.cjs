...
Opting out
  Set "output": { "ios": false } (or android / typescript) in preferences.json to stop generating that
  file. Files without the "Generated by nativescript-preferences" header are never overwritten unless
  --force is given. Set NS_PREFERENCES_SKIP=1 to make the build hook do nothing for one build.
`;
...
function report(projectDir, result, check) {
	...
	for (const file of result.skipped) {
		console.log(`ns-preferences: kept ${relative(projectDir, file)} (hand-written, no generated header; pass --force to overwrite)`);
	}
